...
pub fn main() -> Result<(), JsValue> {
    ...
    // show generated key
    let div_keys = keys_display_node(&document, pk_pair.clone())?;

    // setup hash_comparator
    let div_hash_comparator = hash_comparator_node(&document, pk_pair)?;
...
fn hash_comparator_node(
    ...
    // Label showing data flow from input to encrypted input
    ...
    // label showing data flow from encrypted input to (decrypted) input
    ...
    let handler = Closure::wrap(Box::new(move |_: web_sys::InputEvent| {
        ...
        // TODO: remove unwrap
        let hashed_input = _encrypt(&keys.public_key, hash_input.as_bytes()).unwrap();
        let hashed_input = base64::encode(hashed_input);

        let decrypted_input = base64::decode(hashed_input.clone()).unwrap();
        let decrypted_input = _decrypt(&keys.private_key, &decrypted_input).unwrap();
